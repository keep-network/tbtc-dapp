...
class Client {
  ...
  */
   ...
   * It passes status of the transaction to the callback.
   * @return {string|any} Status of the transaction as defined in the
   * [Electrum Protocol](https://electrumx.readthedocs.io/en/latest/protocol-basics.html#status).
   * If callback is provided it returns value resolved by the callback.
   */
  async waitForTransactionToScript(script, callback) {
    return new Promise(async (resolve, reject) => {
      ...
      // If transaction for script does not exists, we expect to receive null.
      if (callback) {
      ...
      // If transaction for script doesn't exist yet, wait for notification.
      try {
        ...
        reject(new Error(`failed listening for notification: ${err}`))
      }
    })
  }
